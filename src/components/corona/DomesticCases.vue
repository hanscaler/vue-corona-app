    ...
    <div>
        <ul class="graph-list">
            <li>
                <h3>누적 확진/사망자 추이</h3>
                <ChartGraph 
                :chartOptions="domesticCases" 
                :key="caseKey" />
            </li>
            <li>
                <h3>확진자 대비 사만자</h3>
                <ChartGraph 
                :chartOptions="domesticCompare" 
                :key="compareKey" />
            </li>
        </ul>
    </div>
...
<script>
import ChartGraph from '../chart/ChartGraph.vue';
...
export default {
    ...
    methods: {  
        async fetchDomestic() {
            ...
            const from = moment().subtract(this.monthDifference, "months")
            .format("YYYY-MM-DD");
            const params = {
                to, from
            };
